		...
		try {
			...
			for (int idEndpoint = 0; idEndpoint < nEndpoint; idEndpoint++) {				
				...
				for (int j = 0; j < e.nCaches; j++) {					
					...
					cacheServers.get(idCache).nEndpoints++;
				}
				endpoints.add(e);
			}
			
			// sort cache servers based on their connectivity
					...
					if (cacheServer.freeSize >= o.videoSize) {
						...
						for (int k = 0; k < o.candidateTo.size(); k++) {
							...
							if (!satisfiedBy.containsKey(request)) {
								...
								// satisfiedBy.put(request, caches); it should work even without this, since the object reference is the same
							}
						}						
					}
				}		
