...
def argmax(ingredients):
    ...
    for candidate in (pizza for pizza in range(nof_pizzas) if available[pizza]):
        union = ingredients | pizzas[candidate].ingredients
        total = len(ingredients ^ pizzas[candidate].ingredients)**2 / len(union)    # symmetric difference
        if maximum < total:
            maximum = total
            idx = candidate
    return idx
...
if -1 in teams[-1].pizzas:
    ...
    pizza = team_pizzas[0]
    if len(sys.argv) == 1:
        print(f"Pizza n°{pizza} with {len(pizzas[pizza].ingredients)} ingredients was not delivered", file=sys.stderr)
    else:
        print(f"{sys.argv[1]}: Pizza n°{pizza} with {len(pizzas[pizza].ingredients)} ingredients was not delivered", file=sys.stderr)
    del teams[-1]
